...
int main(int argc, char **argv)
{
    ...
    if(getrlimit(RLIMIT_NOFILE, &rl)<0) perror("getlimit()");

    if((pid=fork())<0) perror("error()");
    else if(pid!=0) return 0; // 부모 프로세스 종료
    ...
    sa.sa_flags = 0;
    if(sigaction(SIGHUP, &sa, NULL) <0) perror("sigaction() : Can't ignore SIGHUP");

    // 프로세스의 워킹 디렉토리를 '/'로 설정
    if(chdir("/")<0) perror("cd()");

    // 프로세스의 모든 파일 디스크립터 열기
    if(rl.rlim_max == RLIM_INFINITY) rl.rlim_max = 1024;

    for(i=0;i<rl.rlim_max;i++) close(i);

    // 파일 디스크립터 0, 1과 2를 /dev/null로 연결
