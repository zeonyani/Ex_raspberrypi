...
#include <string.h>
#include <fcntl.h>
#include <dirent.h> // 디렉터리 조작 함수를 위한 헤더파일
...
int listDir(char *arg)
{
	...
	struct dirent *dirt; // 디렉터리 항목을 위한 구조체 
	...
	while((dirt = readdir(pdir)) != NULL){ // 현재 디렉터리를 읽을 수 있으면 순환
		...
		// 파일의 종류 검사
		if(S_ISDIR(statBuf.st_mode)) permission[0] = 'd';
		else if(S_ISLNK(statBuf.st_mode)) permission[0]='l';
		else if(S_ISCHR(statBuf.st_mode)) permission[0]='c';
		else if(S_ISBLK(statBuf.st_mode)) permission[0]='b';
		else if(S_ISSOCK(statBuf.st_mode)) permission[0] = 's';
		else if(S_ISFIFO(statBuf.st_mode)) permission[0] = 'P';
		else permission[0]='-';
